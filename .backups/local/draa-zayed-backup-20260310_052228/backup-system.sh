...
verify_backups() {
    echo -e "${MAGENTA}════════════════════════════════════════════════════════════════════════════════${NC}"
    echo -e "${MAGENTA}7. VERIFYING BACKUPS${NC}"
    echo -e "${MAGENTA}════════════════════════════════════════════════════════════════════════════════${NC}"
    echo ""
    
    echo -e "${BLUE}Verifying checksums...${NC}"
    
    if [ -f "${REPO_ROOT}/.backups/checksums/${BACKUP_NAME}.sha256" ]; then
        cd "$BACKUP_LOCAL"
        if sha256sum -c "${REPO_ROOT}/.backups/checksums/${BACKUP_NAME}.sha256" > /dev/null 2>&1; then
    ...
    echo ""
    echo -e "${CYAN}Location 1 (Local):${NC}"
    du -sh "$BACKUP_LOCAL"
    echo ""
    
    echo -e "${CYAN}Location 2 (Git LFS):${NC}"
    du -sh "$BACKUP_GIT" 2>/dev/null || echo "Not initialized"
    echo ""
    
    echo -e "${CYAN}Location 3 (Cloud):${NC}"
    du -sh "$BACKUP_CLOUD" 2>/dev/null || echo "Not initialized"
    echo ""
}
